...
AppUser _appUserDeserialize(
  ...
  object.role = _AppUserroleValueEnumMap[reader.readByteOrNull(offsets[2])] ??
      UserRole.admin;
...
extension AppUserByIndex on IsarCollection<AppUser> {
  ...
  AppUser? getByUsernameSync(String username) {
  ...
  Future<List<AppUser?>> getAllByUsername(List<String> usernameValues) {
    final values = usernameValues.map((e) => [e]).toList();
    return getAllByIndex(r'username', values);
  }

  List<AppUser?> getAllByUsernameSync(List<String> usernameValues) {
    final values = usernameValues.map((e) => [e]).toList();
  ...
  Future<Id> putByUsername(AppUser object) {
    return putByIndex(r'username', object);
  }

  Id putByUsernameSync(AppUser object, {bool saveLinks = true}) {
    return putByIndexSync(r'username', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByUsername(List<AppUser> objects) {
    return putAllByIndex(r'username', objects);
  }

  List<Id> putAllByUsernameSync(List<AppUser> objects,
      {bool saveLinks = true}) {
...
extension AppUserQueryWhere on QueryBuilder<AppUser, AppUser, QWhereClause> {
  ...
  QueryBuilder<AppUser, AppUser, QAfterWhereClause> usernameEqualTo(
      String username) {
    return QueryBuilder.apply(this, (query) {
  ...
  QueryBuilder<AppUser, AppUser, QAfterWhereClause> usernameNotEqualTo(
      String username) {
    return QueryBuilder.apply(this, (query) {
      if (query.whereSort == Sort.asc) {
        return query
